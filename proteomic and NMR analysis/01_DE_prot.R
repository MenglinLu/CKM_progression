...
## protein annotation
olink_coding <- "/home/zjia/ukb2/aging_ukb/rawdata/prot/coding143.tsv"
olinkID2prot_raw <- readr::read_tsv(olink_coding)

olinkID2prot <- dplyr::mutate(olinkID2prot_raw, gene=sub(";.*","",meaning) )

olinkID2prot <- tidyr::separate(olinkID2prot_raw, meaning, sep=";", into=c("gene","proteinname"), remove=FALSE ) %>% 
    dplyr::mutate(coding=as.character(coding))

olinkID2prot_short <- olinkID2prot[,c("proteinname", "gene")]
...
prot_tib_tidy <- readRDS("/home/zjia/ukb2/aging_ukb/tidy_data/prot_tib_tidy.RDS")
...
g3_mean_tib <- tibble::rownames_to_column(as.data.frame(g3_mean_mat), "gene") %>% dplyr::left_join(olinkID2prot_short)
...
S1_logFC <- S1_DEA[,c("gene", "fdr", "mean.diff")] %>% dplyr::rename(S1_fdr=fdr, S1_logFC=mean.diff)
S2_logFC <- S2_DEA[,c("gene", "fdr", "mean.diff")] %>% dplyr::rename(S2_fdr=fdr, S2_logFC=mean.diff)
S3_logFC <- S3_DEA[,c("gene", "fdr", "mean.diff")] %>% dplyr::rename(S3_fdr=fdr, S3_logFC=mean.diff)
S4_logFC <- S4_DEA[,c("gene", "fdr", "mean.diff")] %>% dplyr::rename(S4_fdr=fdr, S4_logFC=mean.diff)


all_subtype_average <- all_subtype_DEA[,c("gene", "C_average", "S1_average", 
                                      "S2_average", "S3_average", "S4_average")]

metabolite_lib <- readxl::read_xlsx("./data/labtest_metabolome.xlsx", sheet="prot1") %>%
...
logFC_fdr <- dplyr::left_join(S1_logFC, S2_logFC) %>% 
    ...
    dplyr::left_join(all_subtype_average) %>% 
...
logFC_fdr_sig <- dplyr::filter(logFC_fdr, abs(S1_logFC) > cutoff | abs(S2_logFC) > cutoff | 
    ...
    dplyr::arrange(Group) %>% dplyr::left_join(olinkID2prot_short)
...
rownames(mat0) <- rownames(mat1) <- rownames(mat2) <- logFC_fdr_sig$gene

col_fun = colorRamp2(c(-1.2, 0, 1.15), c("#F0E442", "white", "#E69F00"))

ht0 <- Heatmap(mat0, name="abundance", left_annotation=rowAnnotation(Tissue=logFC_fdr_sig$Group,
                                                                     col=list(Group=col) ),
...
draw(ht0+ht, padding = unit(c(2, 10, 2, 2), "mm"))
...
openxlsx::write.xlsx(res_xlsx, file="./results/DEA_prot.xlsx", asTable=TRUE )

save.image("./results/01_DE_prot.RData")
