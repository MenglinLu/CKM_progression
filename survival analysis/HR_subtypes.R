...
data_all <- read.csv('E:/CKM1/GMM_mets_K4/result_s3.csv')
data <- data_all[data_all$Flag_disease==1,]

data_control <- data_all[data_all$Flag_disease==0,]

data_surv <- read.csv('E:/CKM1/data/data_forSurv_all_0407_mortality.csv')
...
for(task in c('hf','af','chd','atherosclerosis','stroke_haemorr','stroke_infarc',
              'cvd','death')){
  ...
  data_subtype5_task <- data_temp_task[data_temp_task$ml_subtype==4,]
...
write.csv(result_df,'E:/CKM1/GMM_mets_K4/HR_subgroup_control_reference_covariate-death.csv')