...
data <- read.csv('E:/CKM1/data/data_forSurv_all_0407_mortality.csv')

kk <- 3

result_k41 <- read.csv(str_c('E:/CKM1/GMM_mets_K4/result_s',as.character(kk),'.csv'))
...
data_merge[data_merge$ml_subtype==4,'ml_subtype'] = 'Subtype 5'
...
data_subtype5_task <- data_temp_task[data_temp_task$ml_subtype=='Subtype 5',]
...
for(subtype_i in c(2,3,4)){
  ...
  HR_p <- paste0("P", ifelse(HR_p < 0.001, " < 0.001", # 若P值<0.001则标记为“<0.001”
                                 paste0(" = ",round(HR_p, 3))))
...
ggsave(str_c('E:/CKM1/GMM_mets_K4/',task,'_k',as.character(kk+1),'_survial_plot-1.png'),p, width = 5, height = 6, dpi = 600, bg='white')
