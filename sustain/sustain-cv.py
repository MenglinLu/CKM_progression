...
data = pd.read_csv('/home/lumenglin/sustain_py/data/data.csv')
...
sustainType = 'mixture_GMM' ### "mixture_GMM", "mixture_KDE", "zscore"
dataset_name = 'data_v8'
...
data[['Phosphate','eGFR','Albumin','HDLc']] = data[['Phosphate','eGFR','Albumin','HDLc']] * -1
...
plt.plot(np.arange(N_S_max,dtype=int),CVIC,marker='o')
plt.xticks(np.arange(N_S_max,dtype=int))
plt.ylabel('CVIC')  
plt.xlabel('Subtypes model') 
plt.title('CVIC')
fig1.savefig(os.path.join(output_folder, 'CVIC.png'),dpi=600)

fig2 = plt.figure(figsize=(6,4))    
df_loglike = pd.DataFrame(data = loglike_matrix, columns = ["s_" + str(i) for i in range(N_S_max)])
df_loglike.boxplot(grid=False)
plt.ylabel('Log likelihood')  
plt.xlabel('Subtypes model') 
plt.title('Test set log-likelihood across folds')
fig2.savefig(os.path.join(output_folder, 'likehood.png'),dpi=600)